 ...
 *
 * [x] Initialize a per-CPU variable,
 * 		 This variable is used to gather the necessary TLB infomation for the vma start,end.
 * 
 * 
 */
void tlb_gather_mmu(struct mmu_gather *tlb, struct mm_struct *mm, unsigned long start, unsigned long end)
{
	...
	/* Is it from 0 to ~0? */
	tlb->fullmm     = !(start | (end+1));  // if fullmm = 1, means cover the full virtual memory.
	tlb->need_flush_all = 0;		// non flush all?
	tlb->local.next = NULL;
	tlb->local.nr   = 0;
...
void tlb_finish_mmu(struct mmu_gather *tlb, unsigned long start, unsigned long end)
{
	...
	tlb_flush_mmu(tlb);		// 1) Flush current TLB 
	...
	for (batch = tlb->local.next; batch; batch = next) {
		next = batch->next;
		free_pages((unsigned long)batch, 0);			// 2) [?] free what pages ? Seems these are physical addr of kernel virtual addr.
	}
	tlb->local.next = NULL;
}
...
again:
	...
	do {
		...
		if (pte_present(ptent)) {
			...
			rss[mm_counter(page)]--;
			page_remove_rmap(page, false);  // unmap the page from PTE.
			if (unlikely(page_mapcount(page) < 0))	// The page_mapcount should be 0, if it's not a shared page.
 ...
 * Tag
 * 
 * [?] ?? Does it need to read/write the physical page ??
 * 
 * partial page ??
 * 
 */
...
int do_swap_page(struct vm_fault *vmf)
{
	...
	entry = pte_to_swp_entry(vmf->orig_pte);			// 1) Read pte to get the swp_entry_t,swp_offset
	...
	page = lookup_swap_cache(entry);					// 2) Check if its physical page is already in swap_cache. indexed by swp_entry_t
	//debug counter
	// Path#1, page is in swap cache.
	if(page){
		hit_on_swap_cache_inc(); // hit on swap cache
	}
	
	// Path#2, page is not in swap cache, read it from block device.
	if (!page) {  
		//
	...
	// [X] No matter we read the page from block device, or the page staied in Swap Cache. It's in Swap Cache now.
	// This record is used to detect if we allocate and copy a new page, not use the swapped in one.
	swapcache = page;	
	locked = lock_page_or_retry(page, vma->vm_mm, vmf->flags);  // [?] Lock the page, means no other thread can touch it?
	 ...
	 *   The page has to stay in the Swap Caceh until here ??
	 * 
	 * [?]  page_private(page) need to == entry.val ?
	 *  if this is a newly allocated page ?
	 * 
	 */
	...
	pte = mk_pte(page, vma->vm_page_prot);   // Create the new pte value based on the physical page info.
	if ((vmf->flags & FAULT_FLAG_WRITE) && reuse_swap_page(page, NULL)) {
		pte = maybe_mkwrite(pte_mkdirty(pte), vma);	// if this is write page_fault, set  _PAGE_DIRTY to the pte.
	...
	if (page == swapcache) {		// Don't copy, use the swapped in page directly.
		...
		activate_page(page);	// Add the page into active LRU  list.
	 ...
	 // 5) free the acquired swp_entry slot.
