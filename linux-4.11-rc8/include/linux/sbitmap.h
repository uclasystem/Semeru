...
struct sbitmap_word {
	 ...
	 */
	unsigned long word;   // 64 bits, in x86_64. 

	/**
	 * @depth: Number of bits being used in @word.
	 */
	unsigned long depth;
} ____cacheline_aligned_in_smp;  // Each sbitmap_word is allocated in a seperate cacheline. For SMP performace.
...
struct sbitmap {
	 ...
	 */
	struct sbitmap_word *map;   // mutiple sbitmap_word 
};

#define SBQ_WAIT_QUEUES 8         // [?] What's the meaning of this ?
...
struct sbitmap_queue {
	 ...
	 * @ws: Wait queues.
	 */
	struct sbq_wait_state *ws;  // [?] each waiting process/thread occupy a sbq_wait_state ??
