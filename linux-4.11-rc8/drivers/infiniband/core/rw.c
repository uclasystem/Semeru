...
/**
 *  Build multiple 1-sided RDMA read/write to get data from client ? 
 * 
 * each entry of scatterlist will lead to one ib_rdma_wr ?
 * 
*/
		...
		u64 remote_addr, u32 rkey, enum dma_data_direction dir)
{
	...
	ctx->map.sges = sge = kcalloc(sg_cnt, sizeof(*sge), GFP_KERNEL);  // allocate multiple ib_sge
	if (!ctx->map.sges)
		goto out;
	...
	for (i = 0; i < ctx->nr_ops; i++) {
		struct ib_rdma_wr *rdma_wr = &ctx->map.wrs[i];  //get a ib_rdma_wr
		u32 nr_sge = min(sg_cnt, max_sge);

		if (dir == DMA_TO_DEVICE)
			rdma_wr->wr.opcode = IB_WR_RDMA_WRITE;					// 1-sided RDMA write 
		else
			rdma_wr->wr.opcode = IB_WR_RDMA_READ;						// 1-sided RDMA read
		rdma_wr->remote_addr = remote_addr + total_len;  // 
		rdma_wr->rkey = rkey;
		rdma_wr->wr.num_sge = nr_sge;
		rdma_wr->wr.sg_list = sge;										// each ib_rdma_wr only has one ib_sge ?
		...
		u64 remote_addr, u32 rkey, enum dma_data_direction dir)
{
	...
	ret = ib_dma_map_sg(dev, sg, sg_cnt, dir); // get dma address of the scatterlist's entries
	if (!ret)
		return -ENOMEM;
	sg_cnt = ret;
