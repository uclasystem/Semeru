...
struct nvmet_rdma_cmd {
	struct ib_sge		sge[2];			// [?] at most 2 ib_sge, how to support scatter/gather ??
	struct ib_cqe		cqe;
	struct ib_recv_wr	wr;				// [?] how can this wr recognize  the ib_rdma_wr or ib_reg_wr ??
	struct scatterlist	inline_sg;   // [?] this should be a scatterlist array ?
...
/**
 * RDMA responds message ?
 * 
 * the data is stored in send_sqe ?
 *  
 */
struct nvmet_rdma_rsp {
	struct ib_sge		send_sge;    // [?] support scatter/gather ?
...
/**
 * allocate enough scatterlist and responds physical page.
 *  
 */
		...
		u32 length)
{
	...
	while (length) {
		...
		sg_set_page(&sg[i], page, page_len, 0);    // let scatterlist[i] points to the newly allocated page.
		length -= page_len;
		i++;
	}
	*sgl = sg;
		...
		struct nvmet_rdma_rsp *r)
{
	...
	r->send_sge.addr = ib_dma_map_single(ndev->device, r->req.rsp,
			sizeof(*r->req.rsp), DMA_TO_DEVICE);   // [?] single ?
...
/**
 * Receive the WR from client.
 * 
 * [?] so, all the data are sent via 2-sided RDMA ?
 *  
 */
...
{
	struct rdma_cm_id *cm_id = rsp->queue->cm_id;
	u64 addr = le64_to_cpu(sgl->addr);					// the mr->iova ? what's this ?
	...
	// Pass the data from recieved wr to newly allocated scatterlist ?
 ...
 * resolve the data,
 * 
 * the data can be multiple discontigous pages ?
 * 
 * 
 * If have multiple discontiguous pages:
 * 	(NVME_KEY_SGL_FMT_DATA_DESC << 4) | NVME_SGL_FMT_INVALIDATE
 * 
 */
...
/**
 * target/server receive a cmd ?
 * nvmet_rdma_rsp is the main data payload ?
 * 
 * DEBUG
 * 
*/
		...
		struct nvmet_rdma_rsp *cmd)
{
	...
	status = nvmet_rdma_map_sgl(cmd);    // get real data from nvmet_rdma_rsp ?
	if (status)
		goto out_err;
...
static void nvmet_rdma_queue_established(struct nvmet_rdma_queue *queue)
{
	...
	while (!list_empty(&queue->rsp_wait_list)) {
		...
		cmd = list_first_entry(&queue->rsp_wait_list,
					struct nvmet_rdma_rsp, wait_list);    // get a nvmet_rdma_rsp from queue->rsp_wait_list ?
