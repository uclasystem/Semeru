		 ...
		 int *resid)
{
	...
	// [?] get reques ? not build ?
	// Build i/o request directly ? not through bio ?
	req = blk_get_request(sdev->request_queue,
			data_direction == DMA_TO_DEVICE ?
			REQ_OP_SCSI_OUT : REQ_OP_SCSI_IN, __GFP_RECLAIM);   // block i/o request is also DMA ? Should not. 
	if (IS_ERR(req))
		return ret;
	rq = scsi_req(req);  // scsi_request is attached behind the kernel request as disk command.
	scsi_req_init(req);
	...
	req->retries = retries;
	req->timeout = timeout;				// usually, the timeout time is 30000  (ms ?)
	req->cmd_flags |= flags;
