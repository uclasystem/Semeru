 ...
 * 
 * 1) bind staging queue and dispatch queue via cpu_id.
 * 2) Register the i/o operation ?
 * 
 */
int blk_register_queue(struct gendisk *disk)
{
	int ret;
	struct device *dev = disk_to_dev(disk);   // device is the partition within this disk ??
	...
	if (q->mq_ops)
		blk_mq_register_dev(dev, q);   // [?] Configure the mapping between staging queue and diaptach queue. 
