...
public:
  ...
  // Declare the  functions in HeapRegion directly.
...
public:
  ...
  BitQueue* _cross_region_ref_target_queue; 

  SyncBetweenMemoryAndCPU(uint hrm_index, G1BlockOffsetTable* bot, HeapRegion* gsp) :
  _top(NULL),       // set in intialization
  _bot_part(bot,gsp), 
  _pre_dummy_top(NULL),
  _cross_region_ref_update_queue(NULL),
  _cross_region_ref_target_queue(NULL)
  {}
...
class HeapRegion: public G1ContiguousSpace {
 ...
 public:
  ...
  //
  // Start of RDMA related structure 
  //
...
public:
 ...
 protected:
  ...
  //mhr: modify
  //record the age of this region (How many times that it survives) (Will choose the min value if several regions evacuated to this region)
  int _age;
 ...
 public:
  ...
  // linked lists.
  HeapRegion* next()              { return _cpu_to_mem_gc->_next; }
  HeapRegion* prev()              { return _cpu_to_mem_gc->_prev; }

  void set_next(HeapRegion* next) { _cpu_to_mem_gc->_next = next; }
  void set_prev(HeapRegion* prev) { _cpu_to_mem_gc->_prev = prev; }
  ...
  //mhr: modify
  //return the minimum ages of all objects in this region.
  int minimum_age() const {
    return _age;
  }

  void cmp_set_age(int age){
...
public:
  ...
  BitQueue* cross_region_ref_target_queue() const{
    return _sync_mem_cpu->_cross_region_ref_target_queue;
  }


  void set_region_cm_scanned()    { _mem_to_cpu_gc->_cm_scanned = true;  }
  void reset_region_cm_scanned()  { _mem_to_cpu_gc->_cm_scanned = false;  _mem_to_cpu_gc->_alive_ratio=0; _mem_to_cpu_gc->_marked_alive_bytes=0;}
  ...
  inline HeapRegionType  region_type()  { return  _cpu_to_mem_gc->_type; }  

  // mapped to which memory server
  int region_to_memory_server_mapping();
  ...
  //mhr: modify
  void send_info_at_gc();
  void send_remset_at_gc();
  void send_target_queue_at_gc();
  void flush_data();
  void read_info_at_gc();
  void read_info_before_gc();
  ...
  void check_cross_region_reg_queue( HeapRegion* hr,  const char* message);

  void check_sync_between_memory_and_cpu(const char* message);
