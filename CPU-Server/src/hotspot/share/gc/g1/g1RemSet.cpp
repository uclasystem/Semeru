...
private:
  ...
  public:
    ...
    static size_t chunk_size() { return 2*M; }  // 2M cards per claim, 1GB space.
  ...
  public:
    ...
    virtual bool do_heap_region(HeapRegion* r) {
      ...
      //if (!r->in_collection_set() && !r->is_empty()) {
...
//mhr: modify
//mhr: new
void G1ScanRSForRegionClosureNew::scan_rem_set_roots(HeapRegion* r) {
  EventGCPhaseParallel event;
  uint const region_idx = r->hrm_index();
  ...
  //HashQueue* tq = r->cross_region_ref_update_queue();
  //TargetObjQueue* to_target_obj_queue = _g1h->heap_region_containing(obj)->target_obj_queue();
  ...
  // for(size_t i = 1; i < tq->_length; i ++) {
  //   if(tq->_queue[i].from == NULL) {
  //     continue;
  //   }
  //   obj = tq->_queue[i].from;
  //   if(_g1h->heap_region_containing(obj) != r) {
  //     continue;
  //   }

  //   if((size_t)obj->klass()<0x400000000000) {
  //     printf("Wrong!");
  //   }
  ...
  //   newobj = _pss->copy_to_survivor_space(in_cset_state, obj, m);
  //   //tq->_queue[i].to = newobj;
  // }
  ...
  // while(!tq->is_empty())
  // {
  //   bool success = tq->pop_local(item);
  //   oop* holder = item;
  //   obj = *holder;
  ...
  //   //mhr: debug
  //   // markOop mnew = obj->mark_raw();
  //   // printf("%d", (mnew->is_marked()));

  //   //RawAccess<IS_NOT_NULL>::oop_store(p, obj);
  //   r->cross_region_ref_update_queue()->push(obj, newobj);
  //   //r->_move_to[obj] = newobj;
...
//mhr: modify
//mhr: new
// void G1ScanRSForRegionClosureNew::scan_strong_code_roots(HeapRegion* r) {
//   EventGCPhaseParallel event;
//   r->strong_code_roots_do(_pss->closures()->weak_codeblobs());
//   event.commit(GCId::current(), _worker_i, G1GCPhaseTimes::phase_name(G1GCPhaseTimes::CodeRoots));
// }
...
bool G1ScanRSForRegionClosure::do_heap_region(HeapRegion* r) {
  ...
  if (_scan_state->set_iter_complete(region_idx)) {
    if(r->is_young() || r->_mem_to_cpu_gc->_cm_scanned) {
...
bool G1ScanRSForRegionClosureNew::do_heap_region(HeapRegion* r) {
  assert(r->in_collection_set(),
         "Should only be called on elements of the collection set but region %u is not.",
         r->hrm_index());
  uint const region_idx = r->hrm_index();
  ...
  //mhr: modify
  //_scan_state->set_iter_complete(region_idx);
  if (!_scan_state->set_iter_complete(region_idx)) {
    return false;
  }
  ...
  G1EvacPhaseWithTrimTimeTracker timer(_pss, _rem_set_root_scan_time, _rem_set_trim_partially_time);
  scan_rem_set_roots(r);
  G1EvacPhaseWithTrimTimeTracker timer_strong(_pss, _strong_code_root_scan_time, _strong_code_trim_partially_time);
  // Scan the strong code root list attached to the current region
...
//mhr: modify
G1ScanRSForRegionClosureUpdate::G1ScanRSForRegionClosureUpdate(G1RemSetScanState* scan_state,
                                                   G1ScanObjsDuringScanRSClosureNew* scan_obj_on_card,
                                                   G1ParScanThreadState* pss,
                                                   G1GCPhaseTimes::GCParPhases phase,
                                                   uint worker_i) :
  _g1h(G1CollectedHeap::heap()),
...
//mhr: modify
bool G1ScanRSForRegionClosureUpdate::claim_card_new(size_t card_index, const uint region_idx_for_card){
  //mhr: restore
  bool success = true;//_ct->set_card_claimed(card_index);
  _scan_state->add_dirty_region(region_idx_for_card);
  return success;
}

void G1ScanRSForRegionClosureUpdate::claim_card(size_t card_index, const uint region_idx_for_card){
  _ct->set_card_claimed(card_index);
  _scan_state->add_dirty_region(region_idx_for_card);
}

void G1ScanRSForRegionClosureUpdate::scan_card(MemRegion mr, uint region_idx_for_card) {
  HeapRegion* const card_region = _g1h->region_at(region_idx_for_card);
  //mhr: modify young
  assert(!card_region->is_young(), "Should not scan card in young region %u", region_idx_for_card);
  card_region->oops_on_card_seq_iterate_careful<true>(mr, _scan_objs_on_card_cl);
...
void G1ScanRSForRegionClosureUpdate::scan_rem_set_roots(HeapRegion* r) {
  EventGCPhaseParallel event;
  uint const region_idx = r->hrm_index();
  if(region_idx == 0){
    printf("0\n");
  ...
  for (size_t current_card = 0; iter.has_next(card_index); current_card++) {
    ...
    // If the card is dirty, then G1 will scan it during Update RS.
    //mhr: why there will be dirty cards???
    //mhr: TODO
    //mhr: debug
    if (_ct->is_card_claimed(card_index) || _ct->is_card_dirty(card_index)) {
      continue;
    ...
    //mhr: modify
    if(claim_card_new(card_index, region_idx_for_card)) {
      MemRegion const mr(card_start, MIN2(card_start + BOTConstants::N_words, top));
...
bool G1ScanRSForRegionClosureUpdate::do_heap_region(HeapRegion* r) {
  assert(r->in_collection_set(),
         "Should only be called on elements of the collection set but region %u is not.",
         r->hrm_index());
  uint const region_idx = r->hrm_index();
...
//mhr: modify
G1ScanRSForRegionClosureMemUpdate::G1ScanRSForRegionClosureMemUpdate(G1RemSetScanState* scan_state,
                                                   G1ParScanThreadState* pss,
                                                   uint worker_i) :
  _g1h(G1CollectedHeap::heap()),
  _pss(pss),
...
bool G1ScanRSForRegionClosureMemUpdate::do_heap_region(HeapRegion* r) {
  // assert(r->in_collection_set(),
  //        "Should only be called on elements of the collection set but region %u is not.",
  //        r->hrm_index());
  uint const region_idx = r->hrm_index();
  ...
  // Do an early out if we know we are complete.
  if (_scan_state->set_iter_complete(region_idx)) {
    G1EvacuationRootClosures* root_cls = _pss->closures();
...
void G1RemSet::scan_rem_set(G1ParScanThreadState* pss, uint worker_i) {
  ...
  //mhr: new
  //
  //G1ScanRSForRegionClosureNew cl(_scan_state, &scan_cl, pss, G1GCPhaseTimes::ScanRS, worker_i);
...
//mhr: modify
void G1RemSet::scan_rem_set_source(G1ParScanThreadState* pss, uint worker_i) {
  //mhr: modify
  //mhr: new
  G1ScanObjsDuringScanRSClosureNew scan_cl(_g1h, pss);
  G1ScanRSForRegionClosureUpdate cl(_scan_state, &scan_cl, pss, G1GCPhaseTimes::ScanRS, worker_i);
  _g1h->collection_set_iterate_from(&cl, worker_i);
...
//mhr: modify
//mhr: new
class G1RefineCardClosureNew: public CardTableEntryClosure {
  G1RemSet* _g1rs;
  G1ScanObjsDuringUpdateRSClosureNew* _update_rs_cl;

  size_t _cards_scanned;
  size_t _cards_skipped;
public:
  G1RefineCardClosureNew(G1CollectedHeap* g1h, G1ScanObjsDuringUpdateRSClosureNew* update_rs_cl) :
    _g1rs(g1h->g1_rem_set()), _update_rs_cl(update_rs_cl), _cards_scanned(0), _cards_skipped(0)
  {}
...
void G1RemSet::update_rem_set(G1ParScanThreadState* pss, uint worker_i) {
  ...
  if (G1HotCardCache::default_use_cache()) {
    ...
    //mhr: new
    //mhr: restore
    //G1ScanObjsDuringUpdateRSClosureNew scan_hcc_cl(_g1h, pss);
  ...
  // Now apply the closure to all remaining log entries.
  {
    ...
    //mhr: new
    //mhr: restore
    //G1ScanObjsDuringUpdateRSClosureNew update_rs_cl(_g1h, pss);
...
void G1RemSet::refine_card_concurrently(jbyte* card_ptr,
  ...
  //mhr: modify young
  // if (!r->is_old_or_humongous_or_archive()&&!r->is_young()) {
  //   return;
  // }
  ...
  *const_cast<volatile jbyte*>(card_ptr) = G1CardTable::clean_card_val();
  ...
  //mhr: modify
  //mhr: new
  G1ConcurrentRefineOopClosureNew conc_refine_cl(_g1h, worker_i);
...
//mhr: new
//mhr: add modify
void G1RemSet::reset_scan_state() {
  _scan_state->reset_iter_states();
}
