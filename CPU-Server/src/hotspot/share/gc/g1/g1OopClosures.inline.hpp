...
inline static void check_obj_during_refinement(T* p, oop const obj) {
  ...
  //mhr: debug
  // printf("from region bottom and end: %llu, %llu\n", (unsigned long long)from->bottom(), (unsigned long long)from->end());
  // printf("p: %llu\n", (unsigned long long)p);

  // if(!from->is_in_reserved(p) && !(from->is_humongous() &&
  //         g1h->heap_region_containing(p)->is_humongous() &&
  //         from->humongous_start_region() == g1h->heap_region_containing(p)->humongous_start_region())) {
  //             printf("in reserved?:%d", from->is_in_reserved(p));
  // }
...
template <class T>
inline void G1ConcurrentRefineOopClosureNew::do_oop_work(T* p) {
  T o = RawAccess<MO_VOLATILE>::oop_load(p);
  if (CompressedOops::is_null(o)) {
    return;
  }
  ...
  //mhr: modify
  HeapRegionRemSet* to_rem_set = _g1h->heap_region_containing(obj)->rem_set();
  BitQueue* to_target_obj_bit_queue = _g1h->heap_region_containing(obj)->cross_region_ref_target_queue();
  //HashQueue* to_target_obj_queue = _g1h->heap_region_containing(obj)->cross_region_ref_update_queue();
  ...
  if (to_rem_set->is_tracked()) {
    ...
    //mhr: modify
    //mhr: new
    if(!_g1h->heap_region_containing(obj)->is_young()&&!_g1h->heap_region_containing(obj)->is_humongous())
      to_target_obj_bit_queue->push(obj);
  }
  else{
    to_rem_set->add_reference(p, _worker_i);
...
template <class T>
inline void G1ScanObjsDuringUpdateRSClosureNew::do_oop_work(T* p) {
  T o = RawAccess<>::oop_load(p);
  if (CompressedOops::is_null(o)) {
    return;
  }
  ...
  assert(to_rem_set != NULL, "Need per-region 'into' remsets.");
  if (to_rem_set->is_tracked()) {
    to_rem_set->add_reference(p, _par_scan_state->worker_id());
...
//mhr: TODO
//mhr: bug because if we do region one by one. Then the object would be somewhere else.
//mhr: but not sure because the newobj would be null
//mhr: modify
template <class T>
inline void G1ScanObjsDuringScanRSClosureOptional::do_oop_work(T* p) {
  T heap_oop = RawAccess<>::oop_load(p);
  if (CompressedOops::is_null(heap_oop)) {
    return;
  }
  ...
  //mhr: temporarily disable this function
  ShouldNotReachHere();
  // oop newobj = _g1h->heap_region_containing(obj)->cross_region_ref_update_queue()->get(obj);
  // if(newobj == NULL) {

  //   //mhr: debug
  //   markOop m = obj->mark_raw();
  //   if (m->is_marked()) {
  //     oop forwardee = (oop) m->decode_pointer();
  //   }
  //   return;
  // }
  // else{
  //   RawAccess<IS_NOT_NULL>::oop_store(p, newobj);
  //   if (!HeapRegion::is_in_same_region(p, newobj)) {
  //     _par_scan_state->enqueue_card_if_tracked(p, newobj);
  //   }
  // }
}
...
//mhr: modify
template <class T>
inline void G1ScanObjsDuringScanRSClosureNew::do_oop_work(T* p) {
  T heap_oop = RawAccess<>::oop_load(p);
  if (CompressedOops::is_null(heap_oop)) {
    return;
  }
  ...
  //   oop newobj = hr->cross_region_ref_update_queue()->get(obj);
  //   if(newobj == NULL) {
  //     return;
  //   }
  //   else{
  //     RawAccess<IS_NOT_NULL>::oop_store(p, newobj);
  //     if (!HeapRegion::is_in_same_region(p, newobj) && !_g1h->heap_region_containing(p)->is_young()) {
  //       _par_scan_state->enqueue_card_if_tracked(p, newobj);
  //     }
  //   }
  // }
...
//mhr: modify
template <class T>
inline void G1ScanRSForOptionalClosureNew::do_oop_work(T* p) {
  _scan_cl->do_oop_work(p);
}
...
void G1ParCopyClosure<barrier, do_mark_object>::do_oop_work(T* p) {
  ...
  const InCSetState state = _g1h->in_cset_state(obj);
  // if((((unsigned long long)obj)>>12) == 0x7fffc0136ULL) {
  //   printf("found old original!: %llu \n", (unsigned long long)obj);
  // }
  if (state.is_in_cset()) {
    oop forwardee;
    markOop m = obj->mark_raw();
  ...
  //   if((((unsigned long long)obj)>>12) == 0x7fffc0136ULL) {
  //   printf("found old not in cset!: %llu \n", (unsigned long long)obj);
  // }
