...
oop G1ParScanThreadState::copy_to_survivor_space(InCSetState const state,
  ...
  if (_old_gen_is_full && dest_state.is_old()) {
    ...
    //assert(false, "No old gen space");
  }
  // if(dest_state.is_old()) {
  //   printf("Old!\n");
  // }
  // if((((unsigned long long)old)>>12) == 0x7fffc0136ULL) {
  //   printf("found old!: %llu \n", (unsigned long long)old);
  // }
  ...
  if (obj_ptr == NULL) {
    ...
    if (obj_ptr == NULL) {
      ...
      if (obj_ptr == NULL) {
        ...
        //mhr: debug
        //assert(false, "Could not allocate in PLAB");
        return handle_evacuation_failure_par(old, old_mark);
      }
    }
  ...
  // if((((unsigned long long)obj_ptr)>>12) == 0x7fffc0136ULL) {
  ...
  if (_g1h->evacuation_should_fail()) {
    ...
    //mhr: debug
    //assert(false, "Should not fail in evacuation");
    return handle_evacuation_failure_par(old, old_mark);
  }
#endif // !PRODUCT
  ...
  if (forward_ptr == NULL) {
    ...
    //mhr: modify 
    //set the new region's age to be the minimum value of all objects' ages.
    //TODO: improve the performance
    HeapRegion* old_region = _g1h->region_at(_g1h->addr_to_region((HeapWord*)old));
    HeapRegion* obj_ptr_region = _g1h->region_at(_g1h->addr_to_region(obj_ptr));
    obj_ptr_region->cmp_set_age(old_region->minimum_age());
    ...
    if (dest_state.is_young()) {
      ...
      if (old_mark->has_displaced_mark_helper()) {
        ...
        // markOop new_mark = old_mark->displaced_mark_helper()->set_age(age)->set_in_target_object_queue(0);
        // old_mark->set_displaced_mark_helper(new_mark);
      } else {
        //mhr: modify
        obj->set_mark_raw(old_mark->set_age(age));
        //obj->set_mark_raw(old_mark->set_age(age)->set_in_target_object_queue(0));
      }
      _age_table.add(age, word_sz);
    } else {
      obj->set_mark_raw(old_mark);
      // obj->set_mark_raw(old_mark->set_in_target_object_queue(0));
    }
