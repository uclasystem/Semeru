...
class G1BlockOffsetTable: public  CHeapRDMAObj<G1BlockOffsetTable, NON_ALLOC_TYPE> {
...
public:
  ...
  // Semeru
  // added by Chenxi
  G1BlockOffsetTablePart(G1BlockOffsetTable* array, HeapRegion* gsp);
  void  initialize_array_offset_par(G1BlockOffsetTable* array, HeapRegion* coverd_region);

  u_char* offset_array_part()  { return _offset_array_part; }
  size_t offset_array_part_length() { return _offset_array_part_length; }
  ...
  //void alloc_existed_block_work(HeapWord* blk_start, HeapWord* blk_end);
