...
// Chenxi.
//
// Degrade the G1ContiguousSpace functions to sub-class, HeapRegion.
...
inline HeapWord*
HeapRegion::block_start_const(const void* p) const {
  return _sync_mem_cpu->_bot_part.block_start_const(p);
}


//
// End of G1ContiguousSpace functions override.
//
...
bool HeapRegion::oops_on_card_seq_iterate_careful(MemRegion mr,
  ...
  do {
    ...
    //mhr: debug
    // if((unsigned long long)obj->klass() == 0xbaadbabebaadbabeULL) {
    //   printf("Wrong here!\n");
    //   assert(false, "wrong!");
    // }
