...
//   allocation._allocated_outside_tlab = true;
//   //mhr: new
//   size_t siz = ((_word_size-1)/64+1)*64;
//   HeapWord* mem = _heap->mem_allocate(siz, &allocation._overhead_limit_exceeded);
//   if (mem == NULL) {
//     return mem;
//   }
//   //mhr: new
//   Universe::heap()->fill_with_dummy_object(mem+_word_size, mem+siz, true);
//   NOT_PRODUCT(_heap->check_for_non_bad_heap_word_value(mem, ((_word_size-1)/64+1)*64));
//   size_t size_in_bytes = ((_word_size-1)/64+1)*64 * HeapWordSize;
//   _thread->incr_allocated_bytes(size_in_bytes);
//   return mem;
// }
...
HeapWord* MemAllocator::allocate_inside_tlab_slow(Allocation& allocation) const {
  ...
  // size_t want_min_tlab_size = ThreadLocalAllocBuffer::compute_min_size(_word_size);
  // size_t min_tlab_size = ((want_min_tlab_size-1)/64+1)*64;
  // mem = _heap->allocate_new_tlab(min_tlab_size, new_tlab_size, &allocation._allocated_tlab_size);
  if (mem == NULL) {
