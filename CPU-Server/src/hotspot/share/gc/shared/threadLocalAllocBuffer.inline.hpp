...
//   // unsafe_max_tlab_alloc is just a hint.
//   const size_t available_size = Universe::heap()->unsafe_max_tlab_alloc(thread()) / HeapWordSize;
//   size_t want_size = desired_size() + align_object_size(obj_size);
//   size_t new_tlab_size = MIN3(available_size/64*64, ((want_size-1)/64+1)*64, max_size());

//   // Make sure there's enough room for object and filler int[].
//   if (new_tlab_size < ((compute_min_size(obj_size)-1)/64+1)*64) {
//     // If there isn't enough room for the allocation, return failure.
//     log_trace(gc, tlab)("ThreadLocalAllocBuffer::compute_size(" SIZE_FORMAT ") returns failure",
//                         obj_size);
//     return 0;
