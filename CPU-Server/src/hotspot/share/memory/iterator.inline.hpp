...
void OopIterateClosure::verify(T* p) {
  if (should_verify_oops()) {
    ...
    if (!CompressedOops::is_null(heap_oop)) {
      ...
      //mhr: debug
      if(!Universe::heap()->is_in_closed_subset(o)) {
        Universe::heap()->is_in_closed_subset(o);
        printf("Not Right!\n");
        printf("%llu %llu %llu\n", (unsigned long long)p, (unsigned long long)p, (unsigned long long)Universe::heap()->base());
      }
