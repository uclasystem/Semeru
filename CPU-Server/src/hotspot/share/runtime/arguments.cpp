...
jint Arguments::parse_each_vm_init_arg(const JavaVMInitArgs* args, bool* patch_mod_javabase, JVMFlag::Flags origin) {
  ...
  for (int index = 0; index < args->nOptions; index++) {
    ...
    // Smeru - Enable or not
      ...
      // Initial = Max for Semeru Memory Pool (CollectedHeap)
      // 1GB at least.
    }else if(match_option(option, "-XX:EnableBitmap")){

        if (FLAG_SET_CMDLINE(bool, EnableBitmap, true) != JVMFlag::SUCCESS) {
          return JNI_EINVAL;
        }
    ...
    // SemeruConcGCThreads
      ...
      ArgsRange errcode = parse_memory_size(tail, &max_rebuild_threshold, 0);  // String, value, min, max= num of core number.
      if (errcode != arg_in_range) {
        jio_fprintf(defaultStream::error_stream(),
                    "Invalid RebuildThreshold number: %s\n", option->optionString);
        describe_range_error(errcode);
        return JNI_EINVAL;
      }

      if (FLAG_SET_CMDLINE(uint, RebuildThreshold, (uint)max_rebuild_threshold) != JVMFlag::SUCCESS) {
        return JNI_EINVAL;
      }
