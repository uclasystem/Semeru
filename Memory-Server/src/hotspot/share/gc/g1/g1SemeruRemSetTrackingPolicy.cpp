...
bool G1SemeruRemSetTrackingPolicy::needs_scan_for_rebuild(SemeruHeapRegion* r) const {
...
/**
 * Set SemeruHeapRegion->_rem_set->_state.
 *  This value decides the behavior of RemSet Rebuilding procesures.
 *  
 */
void G1SemeruRemSetTrackingPolicy::update_at_allocate(SemeruHeapRegion* r) {
  if (r->is_young()) {
...
void G1SemeruRemSetTrackingPolicy::update_at_free(SemeruHeapRegion* r) {
  /* nothing to do */
}

static void print_before_rebuild(SemeruHeapRegion* r, bool selected_for_rebuild, size_t total_live_bytes, size_t live_bytes) {
...
bool G1SemeruRemSetTrackingPolicy::update_humongous_before_rebuild(SemeruHeapRegion* r, bool is_live) {
  ...
  if (is_live && oop(r->humongous_start_region()->bottom())->is_typeArray() && !r->rem_set()->is_tracked()) {
    ...
    selected_for_rebuild = true;          // HeapRegionRemSet->Updating means to rebuild its RemSet ?
  }

  size_t const live_bytes = is_live ? SemeruHeapRegion::SemeruGrainBytes : 0;
...
/**
 * Tag : Choose the Region to Rebuild its RemSet. 
 * Satisfy conditions:
 *    1) Have alive objects
 *    2) Alive ratio is low
 *    3) Not selected yet, SemeruHeapRegion->RemSet->_state == Untracked.
 * 
 * Handles:
 *    Set SemeruHeapRegion->RemSet->_state to Updating.
 */
bool G1SemeruRemSetTrackingPolicy::update_before_rebuild(SemeruHeapRegion* r, size_t live_bytes) {
  assert(SafepointSynchronize::is_at_safepoint(), "should be at safepoint");
  ...
  // Mark objects between ntams -> current top( _top_at_rebuild_starts) as alive.
  ...
  if ((total_live_bytes > 0) &&
    ...
    r->rem_set()->set_state_updating();  // If seclect the Region to rebuild its RemSet, mark it as Updating.
...
/**
 * Tag : After the Rebuild, Set transfer the Updating Regiont to Complete.
 *  
 */
void G1SemeruRemSetTrackingPolicy::update_after_rebuild(SemeruHeapRegion* r) {
  assert(SafepointSynchronize::is_at_safepoint(), "should be at safepoint");
  ...
  if (r->is_old_or_humongous_or_archive()) {
    if (r->rem_set()->is_updating()) {
      ...
      r->rem_set()->set_state_complete();   // Mark SemeruHeapRegion->RemSet->state as Complete.
    }
    ...
    if (r->is_starts_humongous() && !g1h->is_potential_eager_reclaim_candidate(r)) {
      ...
      for (uint j = region_idx; j < (region_idx + size_in_regions); j++) {
        SemeruHeapRegion* const cur = g1h->region_at(j);
    ...
    G1SemeruConcurrentMark* cm = G1SemeruCollectedHeap::heap()->concurrent_mark();
