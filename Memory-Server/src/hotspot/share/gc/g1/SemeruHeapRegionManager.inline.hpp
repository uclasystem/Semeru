...
/**
 * Tag : Commit the free Regions from SemeruHeapRegionManager->_free_list 
 *  
 */
inline void SemeruHeapRegionManager::allocate_free_regions_starting_at(uint first, uint num_regions) {
  _free_list.remove_starting_at(at(first), num_regions);
}

#endif // SHARE_VM_GC_G1_SEMERU_HEAPREGIONMANAGER_INLINE_HPP
