...
#ifndef SHARE_VM_GC_G1_SEMERU_HEAPREGIONSET_INLINE_HPP
#define SHARE_VM_GC_G1_SEMERU_HEAPREGIONSET_INLINE_HPP

#include "gc/g1/SemeruHeapRegionSet.hpp"

inline void SemeruHeapRegionSetBase::add(SemeruHeapRegion* hr) {
  check_mt_safety();
...
inline void SemeruHeapRegionSetBase::remove(SemeruHeapRegion* hr) {
  check_mt_safety();
  verify_region(hr);
...
inline void FreeSemeruRegionList::add_ordered(SemeruHeapRegion* hr) {
...
inline SemeruHeapRegion* FreeSemeruRegionList::remove_from_head_impl() {
  SemeruHeapRegion* result = _head;
  _head = result->next();
  if (_head == NULL) {
    _tail = NULL;
  } else {
...
inline SemeruHeapRegion* FreeSemeruRegionList::remove_from_tail_impl() {
  SemeruHeapRegion* result = _tail;
 ...
 *  
 */
inline SemeruHeapRegion* FreeSemeruRegionList::remove_region(bool from_head) {
  check_mt_safety();
  verify_optional();
