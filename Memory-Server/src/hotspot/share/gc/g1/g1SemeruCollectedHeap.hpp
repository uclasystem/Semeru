...
#ifndef SHARE_VM_GC_G1_G1SemeruCollectedHeap_HPP
#define SHARE_VM_GC_G1_G1SemeruCollectedHeap_HPP
...
//#include "gc/g1/g1ConcurrentMark.hpp"     // Already include.
...
class G1SemeruSTWIsAliveClosure : public BoolObjectClosure {
  G1SemeruCollectedHeap* _g1h;
public:
  G1SemeruSTWIsAliveClosure(G1SemeruCollectedHeap* g1h) : _g1h(g1h) {}
  bool do_object_b(oop p);
};

class G1SemeruSTWSubjectToDiscoveryClosure : public BoolObjectClosure {
  G1SemeruCollectedHeap* _g1h;
public:
  G1SemeruSTWSubjectToDiscoveryClosure(G1SemeruCollectedHeap* g1h) : _g1h(g1h) {}
  bool do_object_b(oop p);
};

class G1SemeruRegionMappingChangedListener : public G1MappingChangedListener {
 private:
...
/**
 * Semeru
 *  The Memory Pool of Memory server.
 *      [x] Heap Organazion,
 *      [x] Concurrent Tracing
 *      [x] STW Compaction
 * 
 * A modified version of G1SemeruCollectedHeap, which inherits from CollectedHeap directly, 
 * not inherit from genCollectedHeap.
 *  
 */
class G1SemeruCollectedHeap : public CollectedHeap {
  // We need some declared class from G1SemeruCollectedHeap.
...
//
public:
  rdma_padding* _debug_meta_region_padding;
...
//   void rebuild_region_sets(bool free_list_only);
  ...
  G1SemeruRegionMappingChangedListener _listener;

  // The sequence of all heap regions in the heap.
  SemeruHeapRegionManager* _hrm;
...
#define assert_at_safepoint_on_vm_thread()                                    \
  ...
  G1EdenRegions _eden;          // G1SemeruCollectedHeap->_eden Region list.
...
//   G1NewTracer* _gc_tracer_stw;
  ...
  // humongous region.
  //
  // Semeru memroy server has humongous Regions, but never allocate related obejcts.
...
//                                              bool* gc_succeeded);
  ...
  // //
  // // Semeru - Initialize the necessay memory server's heap paramters.
  // //          Let memory server know how much space does it need to manage.
  // //
...
// #endif // !PRODUCT
  ...
  G1SemeruSTWIsAliveClosure _is_alive_closure_stw;
  ...
  // discovery.
  G1SemeruCMIsAliveClosure _is_alive_closure_cm;
...
//   DirtyCardQueueSet& dirty_card_queue_set() { return _dirty_card_queue_set; }
  ...
  // Create a G1SemeruCollectedHeap with the specified policy.
  // Must call the initialize method afterwards.
  // May not return if something goes wrong.
  // Constructor
  G1SemeruCollectedHeap(G1SemeruCollectorPolicy* policy);

private:
//   jint initialize_concurrent_refinement();
//   jint initialize_young_gen_sampling_thread();
public:
  // Initialize the G1SemeruCollectedHeap to have the initial and
...
//   static size_t conservative_max_heap_alignment();
  ...
  //
  // Return the type enum of current CollectedHeap
  //  [?] The basic design is that each GC has a dedicated CollectedHeap type.
  //      But in our desing, each GC has 2 kind of CollectedHeap, one for itself, one for the Semeru memory pool.
  ...
  SemeruHeapRegionManager* hrm() const { return _hrm; }
...
//   // But G1SemeruCollectedHeap doesn't yet support this.
...
//   inline SemeruHeapRegion* next_region_in_humongous(SemeruHeapRegion* hr) const;
  ...
  // compete using the SemeruHeapRegionClaimer to claim the regions before
...
//   // Verification
  ...
  // [?] meaning of  this ?? Use the prev_next bitmap as final marking results ?
...
// class G1SemeruParEvacuateFollowersClosure : public VoidClosure {
// private:
//   double _start_term;
//   double _term_time;
...
// public:
//   G1SemeruParEvacuateFollowersClosure(G1SemeruCollectedHeap* g1h,
...
#endif // SHARE_VM_GC_G1_G1SemeruCollectedHeap_HPP
